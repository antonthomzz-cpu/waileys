...
function renderEmptyChatList() {
    chatList.innerHTML = `
        <div id="emptyChats" class="flex h-full flex-col items-center justify-center px-8 text-center">
            <div class="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/[0.06] bg-white/[0.025] shadow-soft">
                <i data-lucide="messages-square" class="h-6 w-6 text-zinc-700"></i>
            </div>
...
function renderSearchResults() {
    ...
    chatList.innerHTML = state.searchResults.map(contact => {
        ...
        return `
            ...
            >
                <div class="waton-avatar-shell flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/[0.06] bg-zinc-900">
                    ${avatarHTML(name, contact.profilePicture, "w-11 h-11")}
                </div>

                <div class="min-w-0 flex-1">
                    <div class="flex min-w-0 items-center gap-2">
                        <h3 class="truncate text-[13px] font-medium text-zinc-300">
                            ${escapeHTML(name)}
                        </h3>

                        ${contact.username ? `
                            <span class="truncate font-mono text-[7px] text-zinc-700">
                                @${escapeHTML(contact.username)}
                            </span>
                        ` : ""}
                    </div>

                    <p class="mt-1 truncate font-mono text-[9px] text-zinc-700">
                        ${escapeHTML(number || jid)}
                    </p>
                </div>

                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-zinc-700 transition group-hover:bg-white/[0.04] group-hover:text-zinc-400">
